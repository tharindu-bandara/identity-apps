...
import {AuthProtocolMetaListItemInterface, FederatedAuthenticatorMetaInterface} from "../models";
...
export class IdentityProviderManagementUtils {
     ...
     */
    public static getAuthenticators(meta: FederatedAuthenticatorMetaInterface[]): Promise<void> {
        return getFederatedAuthenticatorsList()
            .then((response) => {
                // Filter meta based on the available authenticators.
                const filteredMeta = _.intersectionBy(meta, response, "name");

                store.dispatch(
                    setAvailableAuthenticatorsMeta(_.unionBy<FederatedAuthenticatorMetaInterface>(filteredMeta,
                        response, "name"))
                );
            })
            .catch((error) => {
